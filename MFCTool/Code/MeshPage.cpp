...
void MeshPage::TransformPosXSpin(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (!lockOnCheck) {
		...
		*pResult = 0;
		sphere->m_pTransformCom->m_vInfo[Engine::INFO_POS].x = m_fTransformPosX;
...
void MeshPage::TransformPosYSpin(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (!lockOnCheck) {
		...
		*pResult = 0;
		...
		sphere->m_pTransformCom->m_vInfo[Engine::INFO_POS].y = m_fTransformPosY;
		sphere->Set_InitPoint();
		SetDlgItemText(IDC_EDIT15, cVertex);
	}
}
...
void MeshPage::TransformPosZSpin(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (!lockOnCheck) {
		...
		*pResult = 0;
		sphere->m_pTransformCom->m_vInfo[Engine::INFO_POS].z = m_fTransformPosZ;
		sphere->Set_InitPoint();
		SetDlgItemText(IDC_EDIT16, cVertex);
