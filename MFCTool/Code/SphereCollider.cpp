...
_int CSphereCollider::Update_Object(const _float& fTimeDelta)
{
	...
	if (nullptr == m_pParentBoneMatrix)
	{
		...
		const Engine::D3DXFRAME_DERIVED* pFrame = m_pDynamicMesh->m_pMeshCom->Get_FrameByName(m_FrameName.c_str());

		m_pParentBoneMatrix = &pFrame->CombinedTransformationMatrix;

		Engine::CTransform* pPlayerTransCom = m_pDynamicMesh->m_pTransformCom;
