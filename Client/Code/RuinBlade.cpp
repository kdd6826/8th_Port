...
CRuinBlade::CRuinBlade(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT Client::CRuinBlade::Add_Component(void)
{
	...
	// texture
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(Engine::RESOURCE_STAGE, L"Texture_RuinBlade"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
HRESULT CRuinBlade::SetUp_ConstantTable(LPD3DXEFFECT & pEffect)
{
...
CRuinBlade* CRuinBlade::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CRuinBlade*	pInstance = new CRuinBlade(pGraphicDev);
...
HRESULT Client::CRuinBlade::Ready_Object(void)
{
...
Client::_int Client::CRuinBlade::Update_Object(const _float& fTimeDelta)
{
...
void Client::CRuinBlade::Render_Object(void)
{
	...
	if (pPlayerStateCom->playerState != Engine::CPlayerState::STATE_RUINBLADE)
	{
		count = 0;
		return;
	}
