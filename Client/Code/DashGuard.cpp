...
CDashGuard::CDashGuard(LPDIRECT3DDEVICE9 pGraphicDev)
	: CMonster(pGraphicDev)
{

}

CDashGuard::~CDashGuard(void)
{
...
HRESULT Client::CDashGuard::Add_Component(void)
{
	...
	// texture
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(Engine::RESOURCE_STAGE, L"Texture_ConfusionHole1"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
HRESULT CDashGuard::SetUp_ConstantTable(LPD3DXEFFECT & pEffect)
{
...
CDashGuard* CDashGuard::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CDashGuard*	pInstance = new CDashGuard(pGraphicDev);
...
void CDashGuard::Free(void)
{
	Engine::CGameObject::Free();
}


HRESULT Client::CDashGuard::Ready_Object(void)
{
...
Client::_int Client::CDashGuard::Update_Object(const _float& fTimeDelta)
{
	...
	if (!isDash)
	{
		...
		else if (pPlayerStateCom->perfectGuard == true)
		{
			...
			if (m_guardDir == DIR_FRONT)
			{
				...
				playerDir = playerLook * 21.f;
				float radian = atan2f(playerLook2.x, playerLook2.z);
				m_pTransformCom->Set_Rotation(Engine::ROT_Y, radian);
			}
			else if (m_guardDir == DIR_LEFT)
			{
				playerDir = playerRight * 42.f;
...
void Client::CDashGuard::Render_Object(void)
{
